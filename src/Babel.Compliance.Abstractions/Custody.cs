...
public interface ILocalKeyCustodian
{
    ...
    ValueTask<SignatureMaterial> SignAsync(SigningInput input, CancellationToken ct);
    ...
    /// <summary>
    /// يقرأ الشهادة العامة المرتبطة بالمقبض. <b>عامة لا سرّية</b> — الشهادة تُنشر مع كل
    /// مستند موقَّع، فقراءتها لا تفتح شيئاً.
    /// <para/>
    /// <b>لماذا أُضيفت هذه الدالة، وهي إصلاح عيب في العقد لا توسعة لراحة تنفيذ:</b>
    /// كان العقد يحمل <see cref="AttachCertificateAsync"/> للكتابة <b>ولا يحمل قراءة</b>.
    /// وشكل التوقيع الذي تفرضه الهيئة (‏XAdES) يوجب أن تدخل <b>بصمة الشهادة ومُصدِرها
    /// ورقمها التسلسلي داخل الخصائص الموقَّعة</b> — أي داخل البايتات التي سيقع التوقيع
    /// عليها. فالخاتم يحتاج الشهادة <b>قبل</b> أن يوقّع، بينما
    /// <see cref="SignatureMaterial.SignerCertificateDer"/> لا تصله إلا <b>بعد</b> التوقيع.
    /// <para/>
    /// وبلا هذه الدالة يبقى للخاتم مخرجان وكلاهما رديء: أن يوقّع مرتين — توقيع أول
    /// لاستخراج الشهادة ثم توقيع حقيقي، وهو عمل تشفيري مهدور ونافذة لعدم اتساق — أو أن
    /// يقرأ الشهادة من قناة جانبية خاصة بالمزوّد، وهو ما يُبطل غرض العقد أصلاً.
    /// <b>عدم التماثل «كتابة بلا قراءة» هو العيب، والإصلاح إضافي لا كاسر.</b>
    /// </summary>
    /// <returns>بايتات DER للشهادة، أو فارغ إن لم تُربط شهادة بعد.</returns>
    ValueTask<ReadOnlyMemory<byte>> ReadCertificateAsync(CredentialRef credential, CancellationToken ct);
}
...
public interface IProviderKeyCustodian
{
    ...
    /// </summary>
    ValueTask RevokeAsync(CredentialRef credential, CancellationToken ct);
}
