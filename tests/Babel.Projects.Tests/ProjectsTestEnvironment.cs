...
/// المرجعية من <c>data/</c> نفسها، ووحدة مقاولات بمخطّطها الخاص في قاعدة بيانات
...
/// </para>
/// </summary>
internal static class ProjectsTestEnvironment
{
    /// <summary>الجذع الثابت لاسم قاعدة الدفتر — تُلحق به لاحقة هذه العملية.</summary>
    public const string LedgerDatabaseStem = "babel_prj_ledger";

    /// <summary>الجذع الثابت لاسم قاعدة الوحدة — تُلحق به لاحقة هذه العملية.</summary>
    public const string ModuleDatabaseStem = "babel_prj_module";
    ...
    /// <summary>
    /// جذعٌ محجوز لقاعدة مسبار ترقيةٍ لم تُودَع بعد — <b>ومعلنٌ هنا لأن الكنس يجب أن
    /// يعرفه</b>. وأولُ نصّ ترقية لمخطّط هذه الوحدة يجد مكانه محجوزاً في الكنس، فلا
    /// تتسرّب قاعدةُ مسبارٍ ماتت عمليتُها.
    /// </summary>
    public const string UpgradeProbeDatabaseStem = "babel_prj_upgrade_probe";
    ...
    /// </summary>
    public const string AppRole = "babel_prj_app";

    public const int FiscalYear = 2026;

    /// <summary>المستأجر المستعمل في كل هذه الاختبارات.</summary>
    public static TenantId Tenant { get; } = new(new Guid("9a1e9a1e-0000-4000-8000-000000000001"));

    /// <summary>
    /// مستأجرٌ ثانٍ <b>يُشغّل قدرة المحتجز</b> في ملفّ قدراته.
    /// <para>
    /// وهو نصف إثبات القبول: هو وأخوه أدناه لهما دفتر أستاذ مبذور بالكامل وشيفرة واحدة
    /// ومصفوفة واحدة، ولا يفترقان إلا في <b>صفّ ملفّ القدرات</b>. ولو كان الرفض عند
    /// المُطفأ ناتجاً عن نقص في الحساب أو في المصفوفة لسقط عند المُشغَّل أيضاً — فوجود
    /// الاثنين هو ما يجعل الإثبات إثباتاً.
    /// </para>
    /// </summary>
    public static TenantId RetentionEnabledTenant { get; } = new(new Guid("9a1e9a1e-0000-4000-8000-000000000002"));

    /// <summary>مستأجرٌ ثالث <b>يُطفئ قدرة المحتجز</b>، وكل شيء آخر فيه مطابق لأخيه.</summary>
    public static TenantId RetentionDisabledTenant { get; } = new(new Guid("9a1e9a1e-0000-4000-8000-000000000003"));

    /// <summary>كل منشآت هذه المجموعة — مُعلنةً مرّة، فلا تُنسى واحدة عند التأسيس.</summary>
    public static TenantId[] AllTenants { get; } = [Tenant, RetentionEnabledTenant, RetentionDisabledTenant];
    ...
    public static string Maintenance =>
        Environment.GetEnvironmentVariable("BABEL_PRJ_TEST_ADMIN_DB")
    ...
    /// <summary>
    /// يبذر البيانات المرجعية للدفتر: الأدوار، والحسابات، وخريطة الأدوار، والفترات، والعدّاد.
    /// <para>
    /// <b>ولا يبذر ترجمات الأسماء الإنجليزية للأدوار ولا للحسابات — عن قصد.</b> ملفّات
    /// التأليف تحمل عمود <c>name_en</c> بوصفه <b>مصدر تأليف</b>، والمخطّط لا يعرف عموداً
    /// إنجليزياً (ADR-0021): الترجمة صفٌّ في <c>ledger.name_translation</c>. ومجموعةُ هذه
    /// الوحدة <b>لا تقرأ ترجمة حسابٍ ولا دورٍ في بندٍ واحد</b> — إثباتاتها على مبالغ
    /// وأرصدة ورموز رفض — فبذرُها هنا كتابةُ بياناتٍ لا يقرؤها شيء، ونسخُ العمود
    /// الإنجليزي إلى ملفٍّ ثالث بلا حاجة.
    /// </para>
    /// <para>
    /// ومن أراد إثباتاً يقرأ ترجمةً فليبذرها في بنده — <b>لا في تجهيزةٍ يتقاسمها الجميع</b>.
    /// </para>
    /// </summary>
