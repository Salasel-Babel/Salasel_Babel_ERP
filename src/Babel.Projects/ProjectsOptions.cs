...
public sealed class ProjectsOptions
{
    ...
    public const string ConnectionVariable = "BABEL_PROJECTS_DB";

    /// <summary>مفتاح الإعداد المكافئ في الجذر التركيبي.</summary>
    public const string ConfigurationKey = "Babel:Projects:ConnectionString";

    /// <summary>اسم قاعدة التطوير المحلّي — لا يُبلَغ إلا في وضع التطوير المُعلَن.</summary>
    public const string DefaultDatabase = "babel_projects";
    ...
    public void EnsureConfigured()
    {
        if (string.IsNullOrWhiteSpace(ConnectionString))
        {
            throw DeploymentSetting.Missing(
                "projects.connection_not_configured",
                ConnectionVariable,
                ConfigurationKey,
                "اتصال قاعدة المقاولات",
                "the Contracting database connection");
        }
    }
}
