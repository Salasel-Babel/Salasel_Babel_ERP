...
describe("عقد الملاحة للشاشات الأربع", () => {
  ...
  /* كانت هذه تقول «بلا مجموعة» فصارت تقول «ليست في المجموعتين» — والمحروسُ
     واحد. الأربعُ اليوم مجموعةُ قراءةٍ اسمها `postings` («ما بعد الترحيل»)،
     وهي المجموعةُ التي وصفها تعليقُ `sections.ts` منذ كُتبت: «أربعُ أيدٍ
     يجيب كلٌّ منها سؤالاً واحداً». والذي كان يُحرَس ولا يزال أنها **ليست
     خطوةً في سلسلتي المبيعات والمشتريات**: من يقحمها فيهما يُعلّم أن كل
     شراءٍ ينتهي بمرتجع. */
  it("ولا واحدةٌ منها في مجموعتَي المبيعات والمشتريات — فالمرتجع فرعٌ لا خطوة", () => {
    for (const p of LEDGER_PATHS) {
      const group = SCREENS.find((s) => s.path === p)?.group;
      expect(group).toBe("postings");
      expect(group).not.toBe("sales");
      expect(group).not.toBe("purchasing");
    }
