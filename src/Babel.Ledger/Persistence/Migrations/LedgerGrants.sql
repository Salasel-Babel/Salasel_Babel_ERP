...
begin
    ...
    -- ── سجلّ أبعاد العقار: **إدراجٌ وحده يُفتح للتطبيق** ───────────────────
    -- استثناءٌ واعٍ على السطر الذي فوقه، وله وثيقة قرار (المفتاح
    -- the-property-dimension-is-registered-by-the-module-that-creates-the-property).
    -- والحجّة: «البذر بدور المالك» لا يصلح هنا لأن **العقار يُنشئه المستخدم وقت
    -- التشغيل** لا وقت التزويد؛ وبلا صفٍّ في هذا الجدول لا تُقيَّم قاعدة الحجب
    -- GR-RE-001 فيُرفض القيد كاملاً — أي أن الوحدة العقارية معطَّلة تعطيلاً تامّاً.
    --
    -- و**‏update و delete تبقيان مسحوبتين**: تغيير نموذج الملكية بعد التسجيل يُعيد
    -- تفسير قيودٍ سبق ترحيلها بأثر رجعي — سطرٌ كان التزاماً للمالك يصير إيراداً
    -- للشركة بلا أن يتحرّك قيد واحد. والنقل بين النموذجين واقعةٌ لها مستندها.
    execute format('grant insert on ledger.property_dimension to %I', v_role);
