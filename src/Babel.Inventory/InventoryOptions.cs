...
public sealed class InventoryOptions
{
    ...
    public const string ConnectionVariable = "BABEL_INVENTORY_DB";

    /// <summary>مفتاح الإعداد المكافئ في الجذر التركيبي.</summary>
    public const string ConfigurationKey = "Babel:Inventory:ConnectionString";

    /// <summary>اسم قاعدة التطوير المحلّي — لا يُبلَغ إلا في وضع التطوير المُعلَن.</summary>
    public const string DefaultDatabase = "babel_inventory";
    ...
    public void EnsureConfigured()
    {
        if (string.IsNullOrWhiteSpace(ConnectionString))
        {
            throw DeploymentSetting.Missing(
                "inventory.connection_not_configured",
                ConnectionVariable,
                ConfigurationKey,
                "اتصال قاعدة المخزون",
                "the Inventory database connection");
        }
    }
}
