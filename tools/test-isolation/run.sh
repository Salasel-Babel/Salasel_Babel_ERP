...
#
# ‏**‏`--jobs` آمن اليوم.** كان غير آمن لأن مجموعتين تعملان على قواعد بأسماء ثابتة
#    وتُسقطانها عند التهيئة (‏`Babel.Canonicalization.Tests` و`Babel.Compliance.Tests`)،
#    فتُدمّر عمليتان متزامنتان من أيّهما إحداهما الأخرى فيظهر **فشل كاذب**. وقد صارت
#    كل عملية تملك قواعدها وحدها. مقيس بعد الإصلاح: **5 د 18 ث** بأربع عمليات ·
#    543 وحدة · 687 اختباراً · **صفر فشل**، مقابل **16 د 52 ث** تسلسلياً.
