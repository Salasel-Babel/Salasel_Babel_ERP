/* ═══════════════════════════════════════════════════════════════════════════
   ...
   **معرّف العملية المنشورة** وقيمَ الشرائح ومصادرَها. ثم تلتقطه شاشةُ المستند
   ...
   ⚠ **التحذير الذي كان هنا صار مُنفَّذاً، لا موصىً به.** كان يقول: أجسامُ المسوّدات
   تطلب **معرّفات** والصوتُ يحمل **أسماء**، وتحويلُ اسمٍ منطوق إلى معرّفٍ بالتخمين هو
   بالضبط ما يمنعه انضباط هذا المستودع. وكان يبقى تحذيراً لأن `SpokenSlotValue` كانت
   تحمل النصّ فعلاً، فلا شيء يمنع كاتباً من وضعه في موضع `customerId`.

   واليوم شريحةُ الطرف تصل **بمِقبضٍ معتم و`text: null`** (`ResolvedSlotValue`)، فلا
   يوجد اسمٌ يُوضع في موضع معرّف — **لأنه غير موجود في النوع**. والشاشة تفتدي المِقبض
   إلى معرّفٍ أمام عين الإنسان، ثم تنادي العملية. وحارسٌ في الخادم
   (`NoDraftIsBuiltFromASpokenName`) يقيس ذلك على العقد المنشور نفسه.
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
import type { VoiceDispatch, ResolvedSlotValue } from "./command";
...
export interface VoiceDraftField {
  ...
  /**
   * القيمة **نصّاً دائماً** لما ليس طرفاً، و`null` لشريحة الطرف: الطرف يصل
   * **بمِقبضه** لا باسمه.
   */
  ...
  /**
   * **المِقبض المعتم** لشريحة الطرف، و`null` لما عداها. وهو ما تفتديه الشاشة إلى
   * معرّفٍ عند نداء العملية — **ولا يُشتقّ من اسمٍ منطوق بحال**.
   */
...
function fieldOf(value: ResolvedSlotValue, nameAr: string): VoiceDraftField {
  return {
    name: value.name,
    nameAr,
    text: value.text,
