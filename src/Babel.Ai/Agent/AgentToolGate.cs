...
public static class AgentToolGate
{
        ...
        ILookupHandles handles)
    {
        ...
        // يجب أن يكون مِقبضاً معمّى ومُوثَّقاً لهذه الجلسة بعينها (الخطوة السادسة)، ورقمُ
        // هويةٍ يُكتب هناك يسقط عند العلامة لا عند الشكل.
        //
        // ‏**والسبب الأصلي للاستثناء سقط، وبقي سببٌ آخر أضعف أثراً وأصحّ.** كان الأصل أنّ
        // المِقبض نصٌّ عشوائيّ متّصل قد يحمل صدفةً تسع خاناتٍ فيُرفض نداءٌ سليم مرّةً في
        // نحو مئتَي ألف — وهو عطلٌ نادرٌ عشوائيّ لا يُعاد إنتاجه فلا يُصلَح. وذلك أُغلق
        // **بنيوياً** بكتابة المِقبض مجموعاتٍ من ثمانية يفصلها `~` (‏SignedLookupHandles):
        // لا مسار رقميّ فيه يتجاوز ثماني خانات، فلا يطابقه شكلٌ من السبعة بحال.
        // والباقي **دقّةُ الرسالة**: معرّفٌ خام في موضع مِقبض علاجُه «استعمل مِقبضاً من
        // lookup_entity» لا «اكتبه في حقله على الشاشة»، وجملةُ الشكل تقول الثانية.
