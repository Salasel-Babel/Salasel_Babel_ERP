...
public static class VoicePlanGuard
{
    ...
    public static IReadOnlyList<Error> Refuse(VoicePlan plan, Func<string, VoiceIntent?> resolve)
    {
        ...
        for (int index = 0; index < plan.Steps.Count; index++)
        {
            ...
            // ‏**ولا يُشترط أن يسبق الشرطَ خطوة.** كان هنا حارسٌ يقول «شرطٌ على أوّل
            // خطوة عطل»، وسقط على أوّل خطّةٍ حقيقية: «فإن لم تجدها أنشئ لها حساباً»
            // شرطُها على <b>أوّل</b> خطوة، و<b>السؤالُ الذي يجيبه يطرحه الشرطُ نفسه</b>
            // — تسأل الخطّةُ «هل وجدت العميل؟» قبل أن تبدأ. فالشرط قائمٌ بذاته لا تابعٌ
            // لخطوةٍ سبقت، والحارس الخاطئ حُذف بدل أن تُلوى الخطّة له.
