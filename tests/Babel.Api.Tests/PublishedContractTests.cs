...
public sealed class PublishedContractTests
{
    ...
    public async Task العقد_يصف_كل_ما_يحتاجه_فريق_الواجهة_ولا_يصف_باباً_غير_موجود()
    {
        ...
        // ‏**55 عمليةً — والعدد مكتوب بيد عمداً.** عدٌّ مشتقٌّ من الوثيقة نفسها يقارنها
        ...
        //
        // ثم من 38 إلى **55** بسبعة عشر باباً: تسعةٌ لسلسلة المشتريات المخزنية —
        // أمر الشراء وقراءته، والاستلام وقراءته وترحيله، والفاتورة المخزنية،
        // ومرتجع المشتريات بمسوّدته وقراءته وترحيله — وثمانيةٌ لوحدة المخزون:
        // الأصناف تسجيلاً وقراءةً وقائمةً، وحركات المخزون مسوّدةً وقائمةً وترحيلاً،
        // والأرصدة، والتقييم. **ومقيس أنّ الفرق إضافة محضة**: 15 مساراً و23 مخطّطاً
        // مضافة، **صفر محذوف وصفر معدَّل**.
        //
        // ‏**ولماذا نُشرت سلسلة المشتريات كاملةً**: ADR-0044 رفض نشر المرتجع منفرداً
        // بنصّه — «بابٌ لا يوصل إليه بابٌ آخر على هذا السطح يعطي عقداً يَعِد بدورة لا
        // تكتمل». فالدورة تُنشر كاملة أو لا تُنشر.
        Assert.Equal(55, paths.EnumerateObject().SelectMany(static p => p.Value.EnumerateObject())
