...
public sealed class PublishedContractTests
{
    ...
    public async Task العقد_يصف_كل_ما_يحتاجه_فريق_الواجهة_ولا_يصف_باباً_غير_موجود()
    {
        ...
        // والتنزيل بها.
        // ثم من 76 إلى **96** بعشرين باباً لوحدة العقارات — **دورةٌ واحدة كاملة تعمل
        // من طرفها إلى طرفها**: العقار تسجيلاً وقراءةً، والوحدة مورداً فرعياً له
        // وقراءةً، والمستأجر والمالك، والعقد مسوّدةً وقراءةً وجدولَ دفعاتٍ بمعرّفات
        // سطوره وتفعيلاً، وفاتورة الإيجار مسوّدةً وقراءةً وترحيلاً، وسند القبض مسوّدةً
        // وقراءةً وترحيلاً وتخصيصاً، وأعمار المتأخرات ومطابقتها.
        //
        // **وعقد الإيجار ثلاثة أبواب لا أربعة**: لا `…/posting` عليه ولا يجوز أن يوجد —
        // حدث توقيع العقد مُعلَنٌ في المصفوفة بـ`posts_entry=false`، وغيابُ الباب هو ما
        // يجعل «العقد لا يُرحّل» مقروءاً من شكل العقد المنشور لا من تعليق.
        // وما لم يُنشر من الوحدة **نقصٌ مُعلَن** في وثيقة قرارها وفي سجلّ دَين التحقّق،
        // لا صمتٌ: الشيكات والودائع والصيانة ودورة المالك والغرامات والعمولات
        // والتصرّف العقاري لم تُنشر أبوابها في هذا الإيداع.
        //
        ...
        // ثم من 162 إلى **192** بثلاثين باباً لطبقة المخزون التشغيلية: هرم التسكين
        // بخمس عمليات لكل مستوى (مستودع · موقع · رفّ)، والنقل بين موقعين، والأرصدة
        // بتسكينها، وسجلّ وحدات القياس ومعاملات التحويل ومسبارها، ودورة حياة الصنف.
        //
        // ثم من 162 إلى **169** بسبعة أبوابٍ لمساحة عمل الوكيل: فتح جلسة · قراءة حالها
        // كلِّه · إرسال رسالة · قراءة أحداث الدور بمؤشّر · تأكيد خطوة · جواب ورقة السؤال ·
        // قراءة الإنفاق. **وليس فيها بابُ ترحيلٍ واحد ولا يجوز أن يكون**: الوكيل يبلغ
        // المسوّدة وحدها، والترحيل فعلٌ بصريّ يدويّ على شاشة المستند.
        //
        ...
        // ‏origin/develop والعقد هنا، لا بادّعاء. **ومقيسٌ على الدفعة الأخيرة** أن
        // **صفر** مخطّطاً قائماً تغيّر و**صفر** عمليةٍ قائمة تغيّر جسمُها أو استجابتها.
        //
        // ثم من 192 إلى **199** بسبعة أبوابٍ لمساحة عمل الوكيل: فتح جلسة · قراءة حالها
        // كلِّه · إرسال رسالة · قراءة أحداث الدور بمؤشّر · تأكيد خطوة · جواب ورقة السؤال ·
        // قراءة الإنفاق. **وليس فيها بابُ ترحيلٍ واحد ولا يجوز أن يكون**: الوكيل يبلغ
        // المسوّدة وحدها، والترحيل فعلٌ بصريّ يدويّ على شاشة المستند.
        // ثم من 199 إلى **202** بثلاثة أبوابٍ لخدمة المعامِلات: قراءةُ الإصدارات ·
        ...
        // ثم من 206 إلى **208** ببابَي البوّابة الأمامية (ADR-0094): فتحُ جلسةٍ ببريدٍ
        ...
        /* ‏**وبابا البوّابة الأمامية بلا نطاقٍ للسبب نفسه الذي بلا نطاقٍ له إخوتهما**
           (ADR-0094): من يفتح جلسةً ببريدٍ وكلمة مرور **لا منشأة له بعد** يضع معرّفها
           في المسار — البريدُ يقود إلى مستخدمه والمستخدمُ إلى مستأجره. ومن يضبط
           كلمتَه يضبطها **لنفسه**، والهويةُ من الاعتماد وحده — ومسارٌ فيه
           `{userId}` كان سيصير باباً لضبط كلمةِ مرورِ غيرك بتخمين معرّفه. */
        string[] scopeless =
        [
            "/health",
            "/api/v1/session",
            ...
            "/api/v1/access/sessions/password",
        ...
        //
        // ‏**وبابٌ سابع: فتحُ الجلسة ببريدٍ وكلمة مرور** (ADR-0094). وهو الحكمُ نفسه
        // لا استثناءٌ جديد: من يدخل ببريده لا يملك اعتماداً، والبريدُ وكلمتُه في الجسم
        // كما اعتمادُ الانتساب في الجسم. وما زاد أنه **أشدُّ الأبواب تعرّضاً** — يُطرَق
        // بقوائم كلماتٍ شائعة — فهو في `RateLimited` مع إخوته، ورفضُه رمزٌ واحد لثلاث
        // حالات كي لا يصير كشّافَ عملاء. **وضبطُ كلمة المرور ليس منها**: هو مصادَقٌ
        // عليه، والجلسةُ القائمة هي إثباتُ صاحبها.
        string[] anonymous =
        [
            "/health",
            "/openapi/v1.json",
