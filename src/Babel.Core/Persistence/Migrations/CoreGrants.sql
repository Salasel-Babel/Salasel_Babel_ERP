...
begin
    ...
    -- ── معرّفات الدخول بكلمة مرور ──────────────────────────────────────────
    -- **وهنا DELETE ممنوحة، خلافاً لإخوتها أسفل — والسبب مكتوب:** نقلُ معرّفِ دخولٍ
    -- من بريدٍ إلى بريد هو نزعُ الأول وكتابةُ الثاني، ومعرّفان لمستخدمٍ واحد بابان
    -- لحسابٍ واحد: من سحب أحدهما يقرأ «سُحب الوصول» وهو باقٍ على الآخر. والصفُّ
    -- هنا **ليس شهادة** — لا يُبنى عليه كشفُ سرقةٍ ولا تُقرأ منه رواية؛ الشهادةُ
    -- في سجلّ التدقيق بمن ضبط ومتى، وهي لا تُمسّ.
    --
    -- **ولا UPDATE على عمودٍ دون عمود هنا بقصد:** ضبطُ كلمةِ مرورٍ يكتب `proof`
    -- و`set_at` معاً، ويُجيزه `on conflict do update`. ومنحٌ أضيق كان سيجعل النقل
    -- يتمّ بحذفٍ ثم إدراج في كل مرّة، وهو أطولُ نافذةً لا أقصر.
    execute format('grant select, insert, update, delete on core.access_sign_in to %I', v_role);
    execute format('revoke truncate on core.access_sign_in from %I', v_role);
