...
اختبارات PostgreSQL تقرأ اتصال الصيانة من `BABEL_CANON_TEST_ADMIN_DB` وتسقط إلى اتصال
محلي بلا كلمة مرور. **لا كلمات مرور في المستودع.** و**اسم قاعدة الاختبار لا يُقرأ من
بيئة**: يُشتقّ في الشيفرة بلاحقة خاصّة بكل عملية، لأن متغيّراً يثبّت الاسم يُبطل اللاحقة
بصمت ويعيد تدمير التشغيلات المتزامنة بعضها بعضاً
(‏`docs/evidence/traps.md#fakh-test-databases-share-a-fixed-name-across-processes`).
و`BABEL_CANON_TEST_DB` — إن ضُبط — يُقرأ **قالب اتصال** لا اسم قاعدة.
