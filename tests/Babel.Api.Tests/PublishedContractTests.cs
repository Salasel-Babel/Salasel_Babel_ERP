...
public sealed class PublishedContractTests
{
    ...
    public async Task العقد_يصف_كل_ما_يحتاجه_فريق_الواجهة_ولا_يصف_باباً_غير_موجود()
    {
        ...
        // ‏**171 عمليةً — والعدد مكتوب بيد عمداً.** عدٌّ مشتقٌّ من الوثيقة نفسها يقارنها
        ...
        //
        // ثم من 162 إلى **171** بتسعة أبوابٍ لكتالوج المستودعات والمواقع: المستودع
        // تسجيلاً وقائمةً وقراءةً وتعطيلاً وإعادة تفعيل، والموقع **مورداً فرعياً** —
        // تسجيلاً وقائمةً وتعطيلاً وإعادة تفعيل. و**الموقع فرعيٌّ بنيةً لا اتفاقاً**:
        // رمزُ موقعٍ بلا مستودعه ليس هوية، و«A-01» في مستودعين موقعان لا موقع — وهو
        // ما يقوله مفتاح الرصيد الرباعي حرفياً. **ولا `PUT` ولا `DELETE` على أيٍّ
        // منهما**: رمزُ المستودع هو النصّ الذي تحمله كل حركةٍ ورصيدٍ مضى، وتغييرُه
        // يُيتّم كل صفٍّ تاريخي يحمله على جدولٍ لا مسار `UPDATE` إليه.
        //
        ...
        Assert.Equal(171, paths.EnumerateObject().SelectMany(static p => p.Value.EnumerateObject())
