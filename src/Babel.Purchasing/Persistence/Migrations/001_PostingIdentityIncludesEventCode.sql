-- ═══════════════════════════════════════════════════════════════════════════
-- هجرة المشتريات 001 — هوية الترحيل تشمل رمز الحدث
-- ═══════════════════════════════════════════════════════════════════════════
--
-- كان مفتاح uq_purchasing_posting_identity خماسياً: (مستأجر · نوع المستند · معرّفه · رمز الإطلاق · الجيل)،
...
begin
    ...
    if to_regclass('purchasing.document_posting') is null then
        return;
    end if;
    ...
    alter table purchasing.document_posting alter column "EventCode" set not null;
    ...
    -- الجدول أصلاً.
    drop index if exists purchasing.uq_purchasing_posting_identity;

    create unique index uq_purchasing_posting_identity
        on purchasing.document_posting
    ...
    if not exists (
        select 1 from pg_constraint
         where conname = 'ck_purchasing_document_posting_event_code'
           and conrelid = 'purchasing.document_posting'::regclass)
    then
        alter table purchasing.document_posting
            add constraint ck_purchasing_document_posting_event_code check (length(btrim("EventCode")) > 0);
    end if;
end
$migration$;
