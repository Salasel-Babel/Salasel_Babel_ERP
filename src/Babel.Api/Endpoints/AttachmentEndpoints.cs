...
internal static class AttachmentEndpoints
{
    ...
    /// <summary>
    /// فسحةُ <b>تصريف</b> فوق السقف: ما يُسمح باستهلاكه من جسمٍ مرفوض كي يصل الرفض.
    /// <para>
    /// ‏<b>ولماذا وُجدت:</b> رفضٌ يُكتب وجسمُ الطلب ما يزال يصل يصل إلى العميل
    /// <b>اتصالاً مقطوعاً لا جواباً</b>. فبلا فسحةٍ يستهلك فيها الخادم ما تبقّى، يُجهض
    /// حدُّ Kestrel الطلبَ قبل أن يُقرأ الجواب. وهي <b>محدودة عمداً</b>: أربعة
    /// ميبي‌بايت تكفي التجاوز المعتاد ولا تجعل الرفض بابَ قراءةٍ بلا حدّ.
    /// </para>
    /// </summary>
    private const long DrainAllowanceBytes = 4 * 1024 * 1024;
        ...
        CancellationToken cancellationToken)
    {
        ...
        // ── حدّ Kestrel يُرفع **قبل** فحص الطول المُعلَن، لا بعده ──
        //
        // ‏**ولماذا الترتيب هو العطل نفسه:** الفحص أدناه يردّ 413 ويعود فوراً بلا قراءة —
        // وهو الصواب. لكنّ الحدّ العام للسطح (ميغابايت واحد) كان **ما يزال نافذاً** وقتها،
        // فيُجهض Kestrel الطلبَ من تلقائه بينما جوابنا في الطريق، فيقرأ العميل
        // «‏Broken pipe» بدل جسم المشكلة. سباقٌ يمرّ وحده ويسقط تحت الحِمل — وهو ما وقع
        // مقيساً: الاختبار مرّ **3 من 3** منفرداً وسقط في بوّابةٍ كاملة.
        //
        // فالحدّ يُرفع أولاً بفسحة **تصريفٍ** تكفي لاستهلاك ما تبقّى وإيصال الرفض. وهي
        // فسحةٌ محدودة لا فتحُ باب: ما تجاوزها يبقى Kestrel يقطعه، وذلك **مُعلَن** —
        // عميلٌ يرسل أضعاف السقف يُقطع اتصاله، وعميلٌ يتجاوزه بقليل يقرأ 413 بلغتيه.
        if (context.Features.Get<IHttpMaxRequestBodySizeFeature>() is { IsReadOnly: false } limit)
        {
            limit.MaxRequestBodySize = cap + MultipartOverheadBytes + DrainAllowanceBytes;
        }
