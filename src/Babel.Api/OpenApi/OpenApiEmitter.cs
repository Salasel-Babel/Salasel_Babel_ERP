...
internal static class OpenApiEmitter
{
    ...
    [
        .. new List<Operation>
        {
            ...
            // ── سلسلة المشتريات المخزنية والمخزون ────────────────────────────
            //
            ...
            // توجد إلا عن استلام، والاستلام لا يُرحَّل إلا عبر حدّ تقييم المخزون. فالدورة
            // تُنشر كاملة أو لا تُنشر.

            new(ApiRoutes.PurchaseOrders, "post", "draftPurchaseOrder",
                "إنشاء أمر شراء", "Draft a purchase order",
                "يُنشئ أمر شراء — **أول أضلاع المطابقة الثلاثية**: ما طُلب. ولا قيد ولا أثر في الدفتر: أمر الشراء "
                + "التزامٌ تجاري لا واقعة محاسبية.\n\n"
                + "**وكل سطر يحمل وحدة قياسه**: كمّية هذا السطر تصل إلى دفتر المخزون عند الاستلام فتُضرب في تكلفة "
                + "الوحدة، و«عشرة» بلا وحدة ليست معلومة — عشر حبّات أم عشر كراتين؟ والفرق يصل إلى المال بقيدٍ متوازن.",
                "Creates a purchase order — the **first side of the three-way match**: what was ordered. No entry and no "
                + "ledger effect: a purchase order is a commercial commitment, not an accounting fact.\n\n"
                + "**Every line carries its unit of measure**: this line's quantity reaches the inventory subledger at "
                + "receipt and is multiplied by a unit cost, and 'ten' without a unit is not information — ten pieces or "
                + "ten cartons? The difference reaches the money inside a perfectly balanced entry.",
                Body: "PurchaseOrderRequest", Response: "CommercialDocument", Success: 201, Anonymous: false, Query: [],
                ProblemStatuses: [404]),
            ...
            new(ApiRoutes.PurchaseOrder, "get", "readPurchaseOrder",
                ...
                + "**ومورد واحد لا موردان**: «ما حال الأمر؟» و«ما سطوره؟» سؤالان يُجابان بطلبٍ واحد، فلا يفترقان "
                + "عند أول تعديل ولا يحتاج العميل أن يجمعهما بنفسه.",
                "Reads a purchase order with its state, its totals, **and its lines** — the line identifiers are the input "
                + "to a goods receipt.\n\n"
                + "**One resource, not two**: 'what state is the order in?' and 'what are its lines?' are answered by a "
                + "single request, so they cannot drift apart and the client does not have to join them.",
                Body: null, Response: "PurchaseDocumentWithLines", Success: 200, Anonymous: false, Query: [],
                ProblemStatuses: [404]),

            new(ApiRoutes.GoodsReceipts, "post", "draftGoodsReceipt",
                "تسجيل استلام بضاعة مسوّدة", "Draft a goods receipt",
                "يسجّل استلام بضاعة في حالة **DRAFT** — الضلع الثاني: ما وصل. ولا مخزون ولا قيد بعد: الترحيل مورد "
                + "فرعي مستقلّ.\n\n"
                + "وكميةٌ مستلَمة تتجاوز المتبقّي من أمر الشراء **تُرفض** ولا تُقبل بتحفّظ.",
                "Records a goods receipt in state **DRAFT** — the second side: what arrived. No stock movement and no entry "
                + "yet: posting is a separate sub-resource.\n\n"
            ...
            new(ApiRoutes.GoodsReceipt, "get", "readGoodsReceipt",
                ...
                "Reads a goods receipt with its state, its totals, and its lines — the line identifiers are the input to a "
                + "stock bill and to a purchase return.",
                Body: null, Response: "PurchaseDocumentWithLines", Success: 200, Anonymous: false, Query: [],
                ProblemStatuses: [404]),

            new(ApiRoutes.GoodsReceiptPosting, "post", "postGoodsReceipt",
                "ترحيل استلام بضاعة", "Post a goods receipt",
                "يرحّل استلاماً مسوّدة. **وهذا هو الباب الذي تدخل به البضاعة إلى المخزون**: حركةٌ في الدفتر المساعد "
                + "أوّلاً ثم قيدٌ في الدفتر، بهوية ترحيل واحدة على الطرفين — فالواقعة واحدة تُروى مرّتين ولا تُعدّ "
                + "بحبيبيّتين مختلفتين.\n\n"
                + "**وحصين ضد التكرار بهوية الترحيل**: الوصول الثاني بالهوية نفسها يُرجع المستند ذاته و‏alreadyPosted "
                + "= true ورمز 200 بدل 201، ولا يكتب حركةً ثانية ولا قيداً ثانياً — مهما كان ترتيب الوصول.",
                "Posts a draft goods receipt. **This is the door through which goods enter inventory**: a movement in the "
                + "subledger first, then an entry in the ledger, under one posting identity on both sides — one fact told "
                + "twice, never counted at two different granularities.\n\n"
                + "**Idempotent by the posting identity**: a second arrival with the same identity returns the same document "
                + "with alreadyPosted = true and status 200 instead of 201, writing neither a second movement nor a second "
                + "entry — whatever the arrival order.",
                Body: null, Response: "CommercialDocument", Success: 201, Anonymous: false, Query: [],
                ProblemStatuses: [404, 409, 422]),
    ...
    private static IEnumerable<(string Name, Action<Utf8JsonWriter> Write)> Schemas()
    {
        ...
        yield return ("StockLine", static w =>
        {
            w.WriteString("type", "object");
            w.WriteString("description",
                "سطر مستند مشتريات **مخزني** — ومعه وحدة قياسه. والفرق عن سطر المصروف هو الوحدة: كمّية هذا السطر تصل "
                + "إلى دفتر المخزون فتُضرب في تكلفة الوحدة. **ولا حساب فيه ولا رمز حساب.** / "
                + "A **stock** purchasing line — with its unit of measure. What separates it from an expense line is that unit: "
                + "this line's quantity reaches the inventory subledger and gets multiplied by a unit cost. **No account and no "
                + "account code appears in it.**");
            w.WriteStartObject("properties");
            WriteRefProperty(w, "description", "LocalizedText");
            WriteStringProperty(w, "itemGroup", "مجموعة الصنف — مؤهّل الدور.", "The item group — a role qualifier.", 64);
            WriteStringProperty(w, "itemId", "رمز الصنف كما هو في كتالوج المخزون.", "The item code as registered in the inventory catalogue.", 64);
            WriteRefProperty(w, "quantity", "Quantity");
            WriteRefProperty(w, "taxRate", "TaxRate");
            WriteStringProperty(w, "taxClassification", "التصنيف الضريبي: standard · zero · exempt.", "The tax classification: standard, zero, or exempt.", 32);
            WriteStringProperty(w, "unit", "رمز وحدة القياس — وحدة أساس الصنف أو وحدةٌ لها معامل تحويل إليها.", "The unit-of-measure code — the item's base unit, or a unit with a conversion factor into it.", 32);
            WriteRefProperty(w, "unitPrice", "Money");
            w.WriteEndObject();
            WriteRequired(w, "description", "itemGroup", "itemId", "quantity", "taxClassification", "taxRate", "unit", "unitPrice");
            w.WriteBoolean("additionalProperties", false);
        });
        ...
        yield return ("PurchaseOrderRequest", static w =>
        {
            ...
            WriteArrayRefProperty(w, "lines", "StockLine", "السطور. أمرٌ بلا سطر يُرفض.", "The lines. An order with no line is refused.");
            WriteStringProperty(w, "number", "رقم الأمر — فريد داخل المنشأة.", "The order number — unique within the company.", 64);
            WriteDateProperty(w, "orderedOn", "تاريخ الأمر الميلادي.", "The Gregorian order date.");
            WriteStringProperty(w, "supplierId", "معرّف المورد.", "The supplier identifier.", 36);
            WriteStringProperty(w, "warehouseId", "المستودع المستقبِل.", "The receiving warehouse.", 64);
            w.WriteEndObject();
        ...
        yield return ("GoodsReceiptLine", static w =>
        {
            ...
            w.WriteStartObject("properties");
            WriteStringProperty(w, "orderLineId", "معرّف سطر الأمر.", "The order line identifier.", 36);
            WriteRefProperty(w, "quantity", "Quantity");
            w.WriteEndObject();
        ...
        yield return ("GoodsReceiptRequest", static w =>
        {
            ...
            w.WriteStartObject("properties");
            WriteArrayRefProperty(w, "lines", "GoodsReceiptLine", "السطور.", "The lines.");
            WriteStringProperty(w, "number", "رقم الاستلام.", "The receipt number.", 64);
            WriteStringProperty(w, "orderId", "أمر الشراء.", "The purchase order.", 36);
            WriteDateProperty(w, "receivedOn", "تاريخ الاستلام الميلادي.", "The Gregorian receipt date.");
            w.WriteEndObject();
        ...
        yield return ("PurchaseDocumentWithLines", static w =>
        {
            w.WriteString("type", "object");
            w.WriteString("description",
                "مستند مشتريات ومعه سطوره — **مورد واحد لا موردان**: «ما حاله؟» و«ما سطوره؟» سؤالان يُجابان بطلبٍ "
                + "واحد فلا يفترقان. / "
                + "A purchasing document with its lines — **one resource, not two**: 'what state is it in?' and 'what are its "
                + "lines?' are answered by a single request, so they cannot drift apart.");
            w.WriteStartObject("properties");
            WriteRefProperty(w, "document", "CommercialDocument");
            WriteArrayRefProperty(w, "lines", "PurchaseDocumentLine", "السطور بمعرّفاتها.", "The lines with their identifiers.");
            w.WriteEndObject();
            WriteRequired(w, "document", "lines");
            w.WriteBoolean("additionalProperties", false);
        });
