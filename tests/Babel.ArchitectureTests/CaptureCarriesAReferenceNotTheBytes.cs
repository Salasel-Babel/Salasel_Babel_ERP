...
public sealed class CaptureCarriesAReferenceNotTheBytes
{
    ...
    [
        ...
        // ‏**وهذه حالةٌ ثالثة لم يكن في نصّ الحارس اسمٌ لها.** المزدوجة المكتوبة أعلاه —
        // «طرفٌ بعيد» أو «‏base64 في جسم طلب» — تفترض أن المُرمَّز **بايتات مستند**.
        // وما هنا ليس مستنداً ولا جزءاً منه: حمولةٌ ثنائية ثابتة الطول (غرض · منشأة ·
        // شركة · جلسة · موضوع · انتهاء) مُعمّاة بـAES-256-GCM، تُرمَّز base64url لتعبر
        // مسار URL بلا ترميزٍ ثانٍ. لا بايتة من صورةٍ ولا من ملفّ تمرّ من هنا.
        //
        // ‏**وكانت هنا جملةٌ ثانية كاذبة فحُذفت**: «ولا يوجد في هذا الملفّ ما يقبل
        // ‏`byte[]` من مستدعٍ أصلاً» — والمُنشئ `SignedLookupHandles(byte[] signingKey, …)`
        // يفعل ذلك بعينه. والإعفاء قائمٌ على سببه الحقيقي وحده (هذه ليست بايتات مستند)،
        // وسببٌ مكتوب نصفُه خطأ هو ما يلجأ إليه مراجعٌ بعد سنتين.
        //
        // والصياغة منقولة سطراً بسطر عن `Babel.Storage.SignedAttachmentTickets` — وهو
        // خارج نطاق مسح هذا الحارس (‏`src/Babel.Ai/` وحده)، ولذلك لم يظهر من قبل.
        // ‏**ويبقى مُعلَناً هنا لا مُتحايَلاً عليه بواجهةٍ أخرى**: `Base64Url.EncodeToString`
        // كان سيُخرج الملفّ من المسح بلا أن يغيّر شيئاً، وإخراجُ مسارٍ من نظر حارسٍ لأنه
        // أزعجه هو بالضبط ما يجعل الحارس التالي أعمى.
        ("src/Babel.Ai/Lookup/SignedLookupHandles.cs",
            "ليست بايتات مستند: مِقبضٌ موقَّع ثابت الطول (‏HMAC-SHA256) يعبر مسار URL — "
            + "والملفّ لا يقبل byte[] من مستدعٍ إطلاقاً"),
    ];
