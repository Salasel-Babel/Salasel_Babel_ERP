...
using Babel.Sales.NameRegister;
...
public static class SalesModuleRegistration
{
    ...
    public static IServiceCollection AddBabelSales(this IServiceCollection services, Action<SalesOptions> configure)
    {
        ...
        services.AddSingleton<INameRegisterCatalogue, SalesNameRegisters>();
        ...
        foreach (NameRegisterTable table in SalesNameRegisters.Tables)
        {
            NameRegisterTable described = table;

            services.AddSingleton<INameCandidateSource>(provider => new PostgresNameRegister(
                provider.GetRequiredService<SalesOptions>().ConnectionString,
                described,
