...
public sealed class RealEstateOptions
{
    ...
    public const string ConnectionVariable = "BABEL_REALESTATE_DB";

    /// <summary>مفتاح الإعداد المكافئ في الجذر التركيبي.</summary>
    public const string ConfigurationKey = "Babel:RealEstate:ConnectionString";

    /// <summary>اسم قاعدة التطوير المحلّي — لا يُبلَغ إلا في وضع التطوير المُعلَن.</summary>
    public const string DefaultDatabase = "babel_realestate";
    ...
    public void EnsureConfigured()
    {
        if (string.IsNullOrWhiteSpace(ConnectionString))
        {
            throw DeploymentSetting.Missing(
                "realestate.connection_not_configured",
                ConnectionVariable,
                ConfigurationKey,
                "اتصال قاعدة العقارات",
                "the Real-estate database connection");
        }
    }
}
