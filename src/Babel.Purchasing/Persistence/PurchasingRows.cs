...
internal sealed class SupplierBillRow
{
    ...
    /// <summary>
    /// <b>لقطةُ المعامِلات التي حُسبت بها هذه الفاتورة، بشكلها القانوني — أو فراغٌ إن
    /// لم يُستعمل معامِل.</b>
    /// <para>
    /// <b>ولماذا نصٌّ لا مفتاحٌ أجنبي:</b> الإصدارات تعيش في <c>babel_core</c> وهذه
    /// الفاتورة في <c>babel_purchasing</c> (‏<c>deploy/compose.yml</c>)، ومفتاحٌ أجنبيّ
    /// عابرٌ للقواعد غير ممكن في PostgreSQL. واللقطة تحمل <b>معرّف الإصدار والقيم
    /// المستعمَلة معاً</b>، فقارئُ الفاتورة بعد سنتين يقرؤها وحدها.
    /// </para>
    /// <para>
    /// <b>وعمودٌ واحد لا عمودان:</b> معرّفُ الإصدار داخل اللقطة، وعمودٌ ثانٍ يحمله
    /// يستطيع أن يخالفها — فتصير الفاتورة تحمل جوابين عن سؤالٍ واحد. والاستعلام على
    /// الإصدار موضعُه سجلُّ الاستعمال في قاعدة النواة، لا هذا الجدول.
    /// </para>
    /// </summary>
