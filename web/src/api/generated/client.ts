 ...
 * 
 * يسجّل استلام بضاعة في حالة **DRAFT** — الضلع الثاني: ما وصل. ولا مخزون ولا قيد بعد: الترحيل مورد فرعي مستقلّ.
 * 
 * وكميةٌ مستلَمة تتجاوز المتبقّي من أمر الشراء **تُرفض** ولا تُقبل بتحفّظ.
 * 
 * Records a goods receipt in state **DRAFT** — the second side: what arrived. No stock movement and no entry yet: posting is a separate sub-resource.
 * 
 * A received quantity beyond what remains on the purchase order is **refused**, not accepted with a note.
 */
...
export interface DraftPurchaseOrderArgs {
  /** معرّف الشركة. النطاق يُشتق من المسار ويُطابَق بالاعتماد؛ ولا يوجد حقل شركة في الجسم. / The company identifier. Scope comes from the path and is matched against the credential; there is no company field in any body. */
  companyId: string;
  /** جسم الطلب. / The request body. */
...
/**
 * إنشاء أمر شراء / Draft a purchase order
 * 
 * يُنشئ أمر شراء — **أول أضلاع المطابقة الثلاثية**: ما طُلب. ولا قيد ولا أثر في الدفتر: أمر الشراء التزامٌ تجاري لا واقعة محاسبية.
 * 
 * **وكل سطر يحمل وحدة قياسه**: كمّية هذا السطر تصل إلى دفتر المخزون عند الاستلام فتُضرب في تكلفة الوحدة، و«عشرة» بلا وحدة ليست معلومة — عشر حبّات أم عشر كراتين؟ والفرق يصل إلى المال بقيدٍ متوازن.
 * 
 * Creates a purchase order — the **first side of the three-way match**: what was ordered. No entry and no ledger effect: a purchase order is a commercial commitment, not an accounting fact.
 * 
 * **Every line carries its unit of measure**: this line's quantity reaches the inventory subledger at receipt and is multiplied by a unit cost, and 'ten' without a unit is not information — ten pieces or ten cartons? The difference reaches the money inside a perfectly balanced entry.
 */
export async function draftPurchaseOrder(transport: Transport, args: DraftPurchaseOrderArgs, signal?: AbortSignal): Promise<T.CommercialDocument> {
 ...
 * 
 * يرحّل استلاماً مسوّدة. **وهذا هو الباب الذي تدخل به البضاعة إلى المخزون**: حركةٌ في الدفتر المساعد أوّلاً ثم قيدٌ في الدفتر، بهوية ترحيل واحدة على الطرفين — فالواقعة واحدة تُروى مرّتين ولا تُعدّ بحبيبيّتين مختلفتين.
 * 
 * **وحصين ضد التكرار بهوية الترحيل**: الوصول الثاني بالهوية نفسها يُرجع المستند ذاته وalreadyPosted = true ورمز 200 بدل 201، ولا يكتب حركةً ثانية ولا قيداً ثانياً — مهما كان ترتيب الوصول.
 * 
 * Posts a draft goods receipt. **This is the door through which goods enter inventory**: a movement in the subledger first, then an entry in the ledger, under one posting identity on both sides — one fact told twice, never counted at two different granularities.
 * 
 * **Idempotent by the posting identity**: a second arrival with the same identity returns the same document with alreadyPosted = true and status 200 instead of 201, writing neither a second movement nor a second entry — whatever the arrival order.
 */
...
/**
 * قراءة استلام بسطوره / Read one goods receipt with its lines
 * 
 * يقرأ استلاماً بحالته ومجاميعه وسطوره — ومعرّفات السطور مدخل الفاتورة المخزنية ومدخل المرتجع.
 * 
 * Reads a goods receipt with its state, its totals, and its lines — the line identifiers are the input to a stock bill and to a purchase return.
 */
export async function readGoodsReceipt(transport: Transport, args: ReadGoodsReceiptArgs, signal?: AbortSignal): Promise<T.PurchaseDocumentWithLines> {
  const path = "/api/v1/companies/" + encodeURIComponent(args.companyId) + "/goods-receipts/" + encodeURIComponent(args.receiptId) + "";
  const url = path;
  const response = await transport({ method: "GET", url, signal });
  if (!response.ok) throw ProblemError.from(response);
  return decodeSchema(SCHEMAS, "PurchaseDocumentWithLines", response.json) as T.PurchaseDocumentWithLines;
}
...
/**
 * قراءة أمر شراء بسطوره / Read one purchase order with its lines
 * 
 * يقرأ أمر شراء بحالته ومجاميعه **وسطوره** — ومعرّفات السطور هي مدخل الاستلام.
 * 
 * **ومورد واحد لا موردان**: «ما حال الأمر؟» و«ما سطوره؟» سؤالان يُجابان بطلبٍ واحد، فلا يفترقان عند أول تعديل ولا يحتاج العميل أن يجمعهما بنفسه.
 * 
 * Reads a purchase order with its state, its totals, **and its lines** — the line identifiers are the input to a goods receipt.
 * 
 * **One resource, not two**: 'what state is the order in?' and 'what are its lines?' are answered by a single request, so they cannot drift apart and the client does not have to join them.
 */
export async function readPurchaseOrder(transport: Transport, args: ReadPurchaseOrderArgs, signal?: AbortSignal): Promise<T.PurchaseDocumentWithLines> {
