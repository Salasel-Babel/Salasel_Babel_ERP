...
وشُغِّلت اختبارات المعمارية. بالنسخة السابقة من القاعدة (على `e3c8fda`) مرّت **47/47 خضراء**؛ وبالنسخة
الحالية:

```
failed Rule09_TheSolutionMatchesTheModuleMap.EveryProjectOnDiskIsInTheSolution
  مشاريع على القرص وخارج Babel.slnx — لا يبنيها شيء، فمحلّلاتها واختباراتها وأخطاء
  ترجمتها غير مرئية والتكامل المستمر أخضر (traps.md — فخ-43):
...
### القاعدة 10 · لا تحويل يقرأ الثقافة على قيمة تُحفَظ أو تُجزَّأ
`Rule10_NoCultureDependentPersistence.cs`

كل قيمة تُحفَظ أو تُجزَّأ أو تُفهرَس أو تُقارَن أو تُرسَل تُحوَّل إلى نصّ بمزوّد تنسيق
**مذكور صراحةً**. السلسلة المُستكمَلة `$"{at:yyyy-MM}"` تقرأ ثقافة العملية — **وتقويمها**:
‏`2026-08` تصير `1448-03` تحت `ar-SA` (أم القرى)، فتُصدَر فاتورة الشهر **خالية** بلا استثناء
ولا سطر سجل ([`docs/evidence/traps.md` فخ-38](../docs/evidence/traps.md)). ولا `CA1305` ولا
`InvariantGlobalization` يغنيان عنها: الأول لا يرى الشكل المُستكمَل، والثاني يكسر تطبيع NFC
فيهدم سلسلة التجزئة العربية. والتنسيق المحلي للعرض **مسموح** لكن لا يكون ضمنياً أبداً:
وسم `ثقافة-عرض:` بسبب مكتوب يُعفي **ذلك السطر وحده**.

### القاعدة 11 · الشكل القانوني مختوم
`Rule11_TheCanonicalFormIsSealed.cs`

`Babel.Canonicalization` ليست وحدة منتج بل **الطريق الوحيد إلى دالة التجزئة**، والبايتات
المُجزَّأة هي الدليل الذي يُعرض على مدقّق بعد سنوات. ولذلك تُفرض عليها حدود أشدّ من حدود أي
مشروع آخر: **صفر اعتماديات** (ولا حتى النواة المشتركة — ترقية اعتمادية لا يجوز أن تحرّك
بايتة)، و**الدفتر وحده** يجوز أن يعتمد عليها، و**لا مُسلسِل ولا مشغّل قاعدة بيانات** قربها،
و**حارس العولمة الثابتة** موجود ومُستدعى، و**الرابط داخل البايتات** لا في عمود مجاور،
والمتجهات الذهبية مُودَعة.

### القاعدة 12 · محرك الترحيل يحمل الثوابت المقيسة
`Rule12_ThePostingPathIsTheOnlyWriteToTheLedger.cs`

القواعد الإحدى عشرة الأولى تحرس **الحدود**؛ هذه تحرس **السلوك المقيس** داخل الدفتر. كل بند فيها
يقابل فخّاً في `docs/evidence/traps.md` **وقع فعلاً وقيس أثره**، وارتداده لا يُرى في اختبار
وحدة ولا في مراجعة — يُنتج رقماً خاطئاً صامتاً يُكتشف عند التدقيق بعد شهور:

| ما تمنعه | الفخ |
|---|---|
| `SEQUENCE`/`nextval` لأي رقم يراه مدقّق — الترقيم صفّ عدّاد بـ`FOR UPDATE` | فخ-12 |
| `UPDATE` مجرّد على جدول الأرصدة بدل `INSERT … ON CONFLICT DO UPDATE` | فخ-09 |
| صفوف `upsert` غير مرتّبة صراحةً | فخ-10 |
| عبارة كتابة بلا تأكيد عدد الصفوف | فخ-09 · فخ-05 |
| حارس إحكام تصاعدي لكل حساب (`applied_seq <`) | فخ-13 |
| أي `UPDATE`/`DELETE`/`TRUNCATE` على جدول يُضاف إليه فقط | ADR-0002 · ADR-0003 |
| فاصلة عائمة ثنائية في أي مخطّط مالي | فخ-17 |
| معاملة صريحة في مسار الترحيل (أي رحلة محبوسة داخل القفل) | فخ-14 |

> **وملاحظة عن شكل القاعدة نفسها:** هي تفحص **الشيفرة بلا تعليقات**. قاعدة تمنع شكلاً يجب
> أن تفحص ما يُنفَّذ لا ما يُشرح — وإلا أجبرت المهندس على حذف الشرح الذي يبيّن سبب المنع.
