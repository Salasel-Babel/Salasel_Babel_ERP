...
public static class GoldenVectorSet
{
    ...
    [
        ...
        // ══════════════════════ 14. الإصدارات ══════════════════════
        // ⚠ هذا المتجه يقرأ **حالة عامة متغيّرة** (سجلّ الإصدارات)، وهي الحالة
        // الوحيدة في المجموعة كلها التي لا تملكها المجموعة نفسها. عند إدخال v2
        // وفق SPEC §12 صار السجلّ يحوي «v1,v2»، فكان الشكل السابق — الذي يضمّ
        // القائمة كلها — سيُغيّر قيمة متجه v1 المُودَعة، أي أن الإجراء المُوثَّق
        // للترقية يُبطل بنفسه ملفاً مجمَّداً. وادّعاء المتجه المُعلن — وهو المكتوب
        // في وصفه العربي حرفياً — هو «السجلّ يحوي v1 وتبقى سجلات v1 قابلة للتحقق
        // بعد ظهور v2»، لا «السجلّ لا يحوي غير v1». فالادّعاء نفسه محفوظ هنا،
        // والقيمة المُودَعة لم تتحرّك بايتة. والقائمة الكاملة صارت متجهاً في
        // مجموعة v2 (version.registry.lists.v1.and.v2) حيث موضعها الصحيح.
        new("version.registry.contains.v1",
            "سجلّ الإصدارات يحوي v1 — سجلات v1 تبقى قابلة للتحقق بعد ظهور v2",
            // ولا حاشية (note) هنا: الحاشية تُكتب في الملف الذهبي، وكتابتها كانت
            // ستحرّك بصمة بيان v1 المُودَعة. الشرح كلّه في التعليق أعلاه.
            () => Golden.Value(
                CanonRegistry.Versions.Contains("v1", StringComparer.Ordinal) ? "v1" : "V1-UNREGISTERED")),
