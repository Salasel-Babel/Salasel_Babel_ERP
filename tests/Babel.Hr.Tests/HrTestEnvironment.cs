...
/// المرجعية من <c>data/</c> نفسها، ووحدة موارد بشرية بمخطّطها الخاص في قاعدة بيانات
/// منفصلة — تماماً كما يفرض «كل وحدة تملك جداولها».
/// <para>
/// <b>لماذا لا محاكاة للدفتر:</b> ما يجب إثباته هو أن نداءً واحداً على المسيّر يُنتج
/// <b>قيداً لكل قسيمة</b> في دفتر أستاذ حقيقي، وأن كل قيد يحمل <b>طرفه هو</b> في
/// <c>subledger_party_id</c>. ومحاكاة الدفتر تُثبت أن المحاكاة تعمل — بل الأسوأ: العطل
/// الذي تحرسه هذه المجموعة (قيدٌ واحد للمسيّر) <b>متوازن تماماً وسلسلته سليمة</b>،
/// فلا يُظهره إلا فحصُ صفوف <c>ledger.journal_line</c> نفسها.
/// </para>
/// <para>
/// <b>ولا صفّ نِسَبٍ واحد يُبذَر هنا.</b> جدول <c>hr.payroll_settings</c> يُسلَّم فارغاً
/// في الاختبار كما يُسلَّم في الإنتاج؛ وما يحتاج نِسَباً يُودِعها من داخله بمصدرٍ
/// ومعتمِدٍ ظاهرين في نصّ الاختبار، فلا يُقرأ رقمٌ من تهيئةٍ يظنّه قارئٌ إعداداً للمنتج.
/// </para>
/// </summary>
internal static class HrTestEnvironment
{
    /// <summary>الجذع الثابت لاسم قاعدة الدفتر — تُلحق به لاحقة هذه العملية.</summary>
    public const string LedgerDatabaseStem = "babel_hr_ledger";

    /// <summary>الجذع الثابت لقاعدة الموارد البشرية.</summary>
    public const string HrDatabaseStem = "babel_hr_module";
    ...
    public static string HrDatabase { get; } = TestRunScope.Name(HrDatabaseStem);
    ...
    /// </summary>
    public const string AppRole = "babel_hr_app";

    public const int FiscalYear = 2026;

    /// <summary>
    /// منشأة المسيّر: عليها تُبنى المسيّرات وتُرحَّل وتُطابَق. ولها **صفّ نِسَبٍ
    /// معتمد يُودَع من داخل الاختبار نفسه** — لا من بذرٍ في التهيئة — كي يبقى الجدول
    /// المُسلَّم فارغاً وتُقاس رحلةُ الإيداع كما يقيسها المحاسب.
    /// </summary>
    public static TenantId Tenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000001"));

    /// <summary>
    /// منشأة الجدول الفارغ: <b>لا يُودَع فيها صفّ نِسَبٍ واحد أبداً</b>، فيُقاس عليها
    /// الرفض الصريح. ومعزولةٌ لأن إيداع صفٍّ في منشأة المسيّر كان سيجعل هذا الإثبات
    /// يمرّ أو يسقط بترتيب التشغيل لا ببنائه.
    /// </summary>
    public static TenantId EmptyRatesTenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000002"));

    /// <summary>
    /// منشأة نهاية الخدمة: عليها تُستحقّ المخصصات وتُخالَص، بسيناريوهاتها الثلاثة.
    /// ومعزولةٌ لأن ما تتركه — رصيد مخصص لعلاقة عمل منتهية — هو بالضبط ما تفحصه
    /// مطابقةُ منشأة المسيّر.
    /// </summary>
    public static TenantId EndOfServiceTenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000003"));

    /// <summary>
    /// منشأة الحصانة: عليها يُعاد الترحيل بالهوية نفسها ويُثبَت أنه لا يكتب ثانيةً.
    /// </summary>
    public static TenantId IdempotencyTenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000004"));

    /// <summary>منشأة الخصوصية: عليها يُبحث عن الهوية والآيبان والاسم في الدفتر.</summary>
    public static TenantId PrivacyTenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000005"));

    /// <summary>
    /// منشأة الدورة الكاملة: استحقاقٌ ثم صرفٌ ثم سدادُ تأمينات ثم مطابقة. ومعزولةٌ لأن
    /// المطابقة تقرأ **كل** حركات الدفتر المساعد في المنشأة، فمستندٌ يتركه اختبارٌ آخر
    /// يظهر انحرافاً في هذه.
    /// </summary>
    public static TenantId CycleTenant { get; } = new(new Guid("9c2f0d44-0000-4000-8000-000000000006"));

    /// <summary>
    /// منشأة تفرّد الفترة: عليها يُثبَت أن مسيّراً ثانياً للفترة يُرفض <b>في الخدمة</b>
    /// لا في فهرسٍ يفترض جواب المالك.
    /// <para>
    /// <b>ومعزولةٌ لسببٍ مقيس لا احتياطاً:</b> كانت تتقاسم منشأة نهاية الخدمة، فكان
    /// المسيّر — وهو مسحٌ يشمل <b>كل</b> علاقة عمل سارية — يلتقط موظفاً يسجّله اختبارٌ
    /// آخر بتصنيفٍ لا صفَّ نِسَبٍ له، فيُردّ أول مسيّر بـ<c>hr.payroll_settings_missing</c>
    /// قبل أن يبلغ الاختبارُ ما جاء يُثبته أصلاً. سقط <b>2 من 2</b> منفرداً قبل العزل.
    /// وهو بعينه ما تصفه القاعدة أدناه — وقد خُولفت بعد كتابتها.
    /// </para>
    /// </summary>
    ...
    /// وهو بالضبط ما وُجد مسحُ العزل لأجله.
    /// </summary>
    ...
    public static string Maintenance =>
        Environment.GetEnvironmentVariable("BABEL_HR_TEST_ADMIN_DB")
    ...
    {
        foreach (string stem in new[] { HrDatabaseStem, LedgerDatabaseStem })
        {
