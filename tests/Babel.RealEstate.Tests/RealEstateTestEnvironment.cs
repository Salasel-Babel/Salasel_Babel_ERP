...
/// </para>
/// </summary>
internal static class RealEstateTestEnvironment
{
    /// <summary>الجذع الثابت لاسم قاعدة الدفتر — تُلحق به لاحقة هذه العملية.</summary>
    public const string LedgerDatabaseStem = "babel_re_ledger";

    /// <summary>الجذع الثابت لاسم قاعدة الوحدة — تُلحق به لاحقة هذه العملية.</summary>
    public const string ModuleDatabaseStem = "babel_re";
    ...
    /// </summary>
    public const string AppRole = "babel_re_app";

    public const int FiscalYear = 2026;

    /// <summary>
    /// منشأة الدورة الكاملة — <b>نموذج الملكية الذاتية</b>: العقار أصلٌ لها، والأجرة
    /// المفوترة إيرادُ إيجارٍ مؤجَّل لها يُستنفَد بالاستحقاق.
    /// </summary>
    public static TenantId Tenant { get; } = new(new Guid("5a1e5a1e-0000-4000-8000-000000000001"));

    /// <summary>
    /// منشأة ثانية معزولة — <b>نموذج إدارة أملاك الغير</b>.
    /// <para>
    /// وعزلها ليس ترفاً: هي والأولى تشتركان في شيفرة واحدة ومصفوفة واحدة ودفتر واحد،
    /// ولا تفترقان إلا في <b>صفّ نموذج ملكية العقار في سجلّ الدفتر</b>. فوجود الاثنتين
    /// معاً هو ما يجعل «دائن الفاتورة اختلف» إثباتاً لا مصادفة.
    /// </para>
    /// </summary>
    public static TenantId ManagedTenant { get; } = new(new Guid("5a1e5a1e-0000-4000-8000-000000000002"));

    /// <summary>
    /// منشأة ثالثة معزولة تُختبر فيها هوية الإحكام وحصانة الترحيل مباشرةً.
    /// <para>
    /// عزلها لازم: إثبات الحصانة يُرحّل الهوية نفسها مرّتين، وذلك بالضبط ما يجب ألّا
    /// يُخلط بمطابقة دفترٍ مساعد تُقاس على منشأة أخرى.
    /// </para>
    /// </summary>
    public static TenantId GatewayTenant { get; } = new(new Guid("5a1e5a1e-0000-4000-8000-000000000003"));

    /// <summary>
    /// منشأة رابعة <b>لا يُسجَّل فيها عقارٌ في سجلّ أبعاد الدفتر</b> — شاهدُ الرفض.
    /// <para>
    /// وهي التي تُثبت أن غياب صفّ العقار <b>يُعطّل الدورة بصوت عالٍ</b> ولا يمرّ صامتاً.
    /// </para>
    /// </summary>
    public static TenantId UnregisteredTenant { get; } = new(new Guid("5a1e5a1e-0000-4000-8000-000000000004"));

    /// <summary>كل منشآت هذه المجموعة — مُعلنةً مرّة، فلا تُنسى واحدة عند التأسيس.</summary>
    public static TenantId[] AllTenants { get; } =
        [Tenant, ManagedTenant, GatewayTenant, UnregisteredTenant];
