...
public sealed class PublishedContractTests
{
    ...
    public async Task العقد_يصف_كل_ما_يحتاجه_فريق_الواجهة_ولا_يصف_باباً_غير_موجود()
    {
        ...
        // ‏**24 عمليةً — والعدد مكتوب بيد عمداً.** عدٌّ مشتقٌّ من الوثيقة نفسها يقارنها
        // بذاتها فيمرّ على أي إضافة وأي حذف؛ والرقم هنا يُجبر من يوسّع السطح على أن
        // **يمرّ بهذا الملف** فيقرأ سياسة الإصدار قبل أن يوسّعه. ورفعاته الأخيرة:
        // من 16 إلى 17 بإضافة GET /companies/{companyId}/chart-of-accounts، ثم من 17
        // إلى 19 ببابَي التوثيق — GET /openapi/v1.json و GET /docs، ثم من 19 إلى 24
        // بسطح المصادقة: فتحُ الجلسة وتجديدها وإبطالها، وقراءةُ أعضاء المنشأة ودعوةُ
        // عضوٍ إليها. وكلّها إضافات محضة تُبقي v1: لا حقل حُذف، ولا نوع ضُيّق، ولا حقل
        // اختياري صار إلزامياً — قيست بمقارنة الوثيقتين مساراً مساراً ومخطّطاً مخطّطاً.
        Assert.Equal(24, paths.EnumerateObject().SelectMany(static p => p.Value.EnumerateObject())
