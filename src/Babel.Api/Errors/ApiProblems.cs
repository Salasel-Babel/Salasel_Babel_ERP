...
internal static class ApiProblems
{
    ...
    public static int StatusOf(string code)
    {
        ...
        return code switch
        {
            ...
            // ‏**وثلاثةٌ تحت `access.` ليست فشلَ مصادقة بل رفضَ إدخال** (ADR-0094):
            // هي أجوبةُ **ضبطِ** معرّف الدخول وكلمته — بريدٌ ليس بريداً، وطولٌ خارج
            // الحدّين، ومعرّفٌ لغيرك. وصاحبُها **مصادَقٌ عليه**: جلستُه هي التي أوصلته
            // إلى الباب. فردُّها 401 كان سيقول له «اعتمادُك غير مقبول» وهو مقبول،
            // فيخرج ويدخل من جديد ويقع في الشيء نفسه. و422 تقول «فهمتُك ورفضتُ ما
            // كتبتَ» — وهي تُقرأ فتُصلَح. والقائمةُ مقفلةٌ حرفياً لا ببادئة: بادئةٌ
            // مثل `access.set_` كانت ستُخرج من 401 كلَّ رمزٍ يبدأ بها بعد سنة.
            _ when code is "access.handle_shape_rejected"
                        or "access.password_length_rejected"
                        or "access.handle_taken" => 422,
